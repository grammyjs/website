...
Якщо у цьому ЧаПи немає відповіді на ваше запитання, також ознайомтеся з [ЧаПи про ботів](https://core.telegram.org/bots/faq), створеним командою Telegram.
...
2. Перевірте, чи є параметр опціональним.
   Якщо це так, то він знаходиться в об'єкті для необов'язкових опцій під назвою `other`.
   Передайте `{ назва_параметру: значення }` у цьому місці і це спрацює.
...
Таким чином, вам не доведеться турбуватися про екранування незвичайних символів.
Може здаватися, що це потребує більше коду, але це набагато надійніше і більш безпечне рішення цієї проблеми.
...
Коли користувач блокує вашого бота, ви не зможете надсилати йому повідомлення або іншим чином взаємодіяти з ним, за винятком випадків, коли ваш бот був запрошений до групового чату, учасником якого є цей заблокований користувач.
...
grammY розробляється в першу чергу для Deno, але вона так само добре підтримує Node.js.
