...
Для grammY він надає клас під назвою `InlineKeyboard`.

> Кнопки `switchInline` і `switchInlineCurrent` запускають inline запити.
...
Ось три приклади того, як створити вбудовану клавіатуру з кнопками `text`.

Ви також можете використовувати інші методи, такі як `url`, щоб дозволити клієнтам Telegram відкривати URL-адресу, і багато інших опцій, перелічених у [довідці API grammY](https://deno.land/x/grammy/mod.ts?s=InlineKeyboard#Methods), а також [довідці Telegram Bot API](https://core.telegram.org/bots/api#inlinekeyboardbutton) для `InlineKeyboard`.

#### 1-й приклад

Кнопки для навігації по сторінках можуть бути побудовані наступним чином:
...
![1-й приклад](/images/inline-keyboard-example-1.webp)

#### 2-й приклад

Вбудована клавіатура з кнопкою "Поділитися" може бути побудована наступним чином:
...
![2-й приклад](/images/inline-keyboard-example-2.webp)

#### 3-й приклад

Кнопки з посиланням можуть бути побудовані наступним чином:
...
![3-й приклад](/images/inline-keyboard-example-3.webp)
...
Однак відповідь на натискання таким чином може бути стомлюючою.
...
Ви можете прослуховувати дані зворотного виклику за допомогою `bot.callbackQuery()`.
...
`bot.callbackQuery()` корисний для прослуховування подій натискання певних кнопок.
Ви можете використовувати `bot.on("callback_query:data")` для прослуховування подій будь-якої кнопки.
...
> Перегляньте розділ про власну клавіатуру в [можливостях ботів Telegram](https://core.telegram.org/bots/features#keyboards), написаний командою Telegram.
...
Для цього він надає клас під назвою `Keyboard`.

Як тільки користувач натисне кнопку `text`, ваш бот отримає надісланий текст у вигляді звичайного текстового повідомлення.
Пам'ятайте, що ви можете прослуховувати текстові повідомлення за допомогою `bot.on("message:text")` або `bot.hears()`.

### Побудова власної клавіатури

Ось три приклади того, як створити власну клавіатуру з кнопками `text`.

Ви також можете запитувати номер телефону за допомогою `requestContact`, місцезнаходження за допомогою `requestLocation`, опитування за допомогою `requestPoll`, користувача за допомогою `requestUser` і чат за допомогою `requestChat`.

#### 1-й приклад

Три кнопки в одній колонці можуть бути побудовані наступним чином:
...
![1-й приклад](/images/keyboard-example-1.webp)

#### 2-й приклад

Калькулятор можна побудувати наступним чином:
...
![2-й приклад](/images/keyboard-example-2.webp)

#### 3-й приклад

Чотири кнопки в решітці можуть бути побудовані наступним чином:
...
![3-й приклад](/images/keyboard-example-3.webp)
...
За замовчуванням користувачі бачать іконку, яка дозволяє їм показувати або приховувати власну клавіатуру, встановлену вашим ботом.
...
Зазвичай, клавіатура завжди матиме розмір стандартної клавіатури програми.
...
#### Одномоментні власні клавіатури

Ви можете викликати `oneTime`, якщо хочете, щоб власна клавіатура була прихована одразу після натискання першої кнопки.
...
Якщо ви хочете обробити всі натискання кнопок в одному обробнику, використовуйте `bot.on("message:text")` і перевірте `ctx.msg.text`, щоб зрозуміти, яка кнопка була натиснута або що було надіслано звичайне текстове повідомлення.

### Видалення власної клавіатури

Якщо не вказати `one_time_keyboard`, як описано [вище](#одномоментні-власні-клавіатури), власна клавіатура залишатиметься відкритою для користувача, але
