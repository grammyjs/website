...
Для цього користувачі починають повідомлення з `@імʼя_вашого_бота` і вибирають один із результатів.
...
Зробили це?
Тепер ваш клієнт Telegram повинен відображати "...", коли ви вводите імʼя бота в будь-якому текстовому полі, й показувати завантажувальний спінер.
Ви вже можете починати щось вводити.
Тепер давайте подивимося, як ваш бот може обробляти ці запити.

## Обробка inline-запитів

Як тільки користувач запускає iline-запит, тобто починає друкувати повідомлення, вводячи `@імʼя_вашого_бота ...` у полі для введення тексту, ваш бот отримає оновлення про це.
...
2. _Обʼєкт результату_, який описує, як відобразити результат inline-запиту.
   Він може містити заголовок, посилання або зображення.
...
Діалог може трохи прокручуватися вперед і назад, перш ніж ви відправите користувача назад.
Наприклад, ви можете [ввести коротку розмову](./conversations.md#встановлення-та-вхід-до-розмови) за допомогою плагіна розмов (`conversations`).
