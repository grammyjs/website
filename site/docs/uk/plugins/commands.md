...
- Функція `Може, ви мали на увазі ...?`, яка знаходить найбільш схожу існуючу команду до введеної користувачем помилкової команди
...
Зверніть увагу на різний підхід до реєстрації, який використовується у файлах `admin.ts` та `group.ts`.
...
Помітили, що в екземплярі `CommandGroup` можна реєструвати окремі ініціалізовані команди за допомогою методу `.add` або ж безпосередньо за допомогою методу `.command(...)`?
...
Плагін команд допоможе вам впоратися з цим, дозволяючи запропонувати команду, яка може бути саме тією, яку користувач хотів ввести, але помилився.
