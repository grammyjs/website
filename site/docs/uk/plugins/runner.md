...
Це корисно при встановленні [проміжного обробника сесії](./session.md), але це також гарантує, що ваш бот не переплутає порядок повідомлень в одному чаті.

Runner експортує проміжний обробник `sequentialize`, який піклується про це.
Ви можете переглянути цей [розділ](../advanced/scaling.md#паралелізм-складнии), щоб дізнатися, як ним користуватися.
...
Це дивовижно, тому що [паралелізм складний](../advanced/scaling.md#паралелізм-складнии), а це означає, що якщо є лише один потік, знімається багато головного болю.
