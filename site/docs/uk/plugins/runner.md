...
Runner складається з трьох частин: джерела оновлень (source), обробника оновлень (sink) та runner'а.
Джерело підтягує оновлення, поглинач надає оновлення, а runner налаштовує та зʼєднує ці дві частини.

> Детальний опис того, як працює бігун, можна знайти [тут](#як-це-працює-за-кулісами).

Кожну з цих трьох частин можна налаштувати за допомогою різних опцій.
Це може зменшити мережевий трафік, дозволити вам вказати дозволені оновлення тощо.
...
run(bot, {
  ...
  sink: {}, // параметри обробника оновлень
});
```
...
По суті, єдине ядро почне боротися з обробкою JSON всіх повідомлень, які повинен обробляти ваш бот.
...
// Запускаємо бота паралельно з багатопоточністю.
run(bot);
```
