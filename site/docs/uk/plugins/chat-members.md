...
Цей плагін полегшує роботу з об'єктами `ChatMember`, пропонуючи зручний спосіб відстеження змін у вигляді власних фільтрів і зберігаючи та оновлюючи об'єкти.

## Вступ

Робота з об'єктами `ChatMember` з Telegram Bot API іноді може бути громіздкою.
...
Об'єднайте ці фільтри з `bot.chatType`, щоб прослуховувати переходи лише для певного типу чату.
...
### Гідратація об'єктів учасників чату

Ви можете ще більше покращити свій досвід розробки, використовуючи гідратацію [перетворювача API](../advanced/transformers).
Цей перетворювач застосовуватиметься до викликів `getChatMember` і `getChatAdministrators`, додаючи зручний метод `is` до об'єктів `ChatMember`, що повертаються.
