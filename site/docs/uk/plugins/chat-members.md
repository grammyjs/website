...
- Оновлення `chat_member` надходять лише в тому випадку, якщо ви явно включили їх в список дозволених оновлень, вони повідомляють про будь-які зміни статусів користувачів в чатах, в яких бот є **адміністратором**.
...
groups.filter(myChatMemberFilter("admin", "regular"), async (ctx) => {
  await ctx.reply("Я більше не адміністратор");
});
