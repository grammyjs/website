...
> [Стрибайте вниз](#як-використовувати-сесіі), якщо ви знаєте, як працюють сесії.
...
Звісно, grammy має все необхідне для цього, тому вам не потрібно розміщувати це самостійно.
...
> Якщо ви хочете запустити власну базу даних, можете бути впевнені, що grammY підтримує це так само добре.
...
Наприклад, припустимо, ми хочемо створити бота, який підраховує кількість разів, коли текст повідомлення містить емодзі піци :pizza:.
...
Фактично, ваш бот буде зберігати карту від ідентифікатора чату до деяких власних даних сесії, тобто щось на кшталт цього:
...
> Це стосується файлів, хмарних сховищ чи будь-чого іншого.

Гаразд, але що таке наразі сесії?

Ми можемо встановити middleware на бота, який буде надавати дані про сесію чату в `ctx.session` при кожному оновленні.
Встановлений плагін буде робити щось до і після виклику наших обробників:

1. **До middleware обробника.**
...
2. **Під час виконання middleware обробника.**
   Ми можемо _прочитати_ `ctx.session`, щоб перевірити, яке значення було в базі даних.
   Наприклад, якщо в чат буде надіслано повідомлення з ідентифікатором `424242`, це буде `ctx.session = { pizzaCount: 24 }` під час роботи нашого middleware; принаймні, з наведеним вище прикладом стану бази даних.
   Ми також можемо довільно модифікувати `ctx.session`, тобто додавати, видаляти та змінювати поля, як нам заманеться.
3. **Після middleware обробника.**
   Middleware сесії гарантує, що дані будуть записані назад до бази даних.
   Яким би не було значення `ctx.session` після завершення роботи middleware обробника, воно буде збережено в базі даних.
...
| _Формат_          | будь-які обʼєкти JavaScript: рядки, числа, масиви тощо | будь-який формат: двійкові або структуровані дані, файли тощо            |
...
Це не означає, що все _не може працювати_, якщо ви обираєте сесії або бази даних.
Наприклад, ви звісно можете зберігати великі двійкові дані в сесії.
...
Ви можете додати підтримку сесії до grammY за допомогою вбудованого middleware сесії.
...
// Встановлюємо middleware сесії та визначаємо початкове значення сесії.
...
Зверніть увагу, що нам також потрібно [налаштувати тип контексту](../guide/context.md#налаштування-обєкта-контексту), щоб зробити сесію доступним у ньому.
Flavor для контексту називається `SessionFlavor`.

### Початкові дані сесії

Коли користувач вперше контактує з вашим ботом, йому не доступні дані сесії.
Тому важливо вказати параметр `initial` для middleware сесії.
...
Ви можете вказати, який ключ сесії використовувати, передавши функцію `getSessionKey` до [налаштувань](https://deno.land/x/grammy/mod.ts?s=SessionOptions#prop_getSessionKey).
Отже, ви можете кардинально змінити спосіб роботи плагіна сесії.
За замовчуванням дані зберігаються для кожного чату.
...
Переконайтеся, що ви розумієте наслідки такої конфігурації, прочитавши [цю статтю](../guide/deployment-types.md) й особливо [цю](./runner.md#послідовна-обробка-там-де-це-необхідно).
:::
...
- Ігнорувати проблему.
  ...
  Наприклад, якщо міграція відбувається під час розмови, яка ведеться за допомогою [плагіна розмов](./conversations.md), розмова буде скинута.
...
- Зберігати в сесії лише тимчасові або обмежені тайм-аутами дані, а важливі дані, які потрібно перенести під час міграції чату, використовувати базу даних.
  ...
  Багато зусиль і має вартість виконання, але це єдиний дійсно надійний спосіб вирішити цю проблему.
...
- Теоретично можливо реалізувати обхідний шлях, який відповідає обом чатам **без гарантії надійності**.
  ...
  Проблема в тому, що немає гарантії, що ці повідомлення будуть надіслані до того, як з'явиться нове повідомлення в супергрупі.
...
- Іншим обхідним шляхом може бути обмеження бота лише для супергруп за допомогою [фільтрації](../guide/filter-queries.md) або обмеження для супергруп функцій, повʼязаних лише з сесією.
...
Це зручно, коли ви розробляєте бота або запускаєте автоматичні тести, адже не потрібно налаштовувати базу даних, однак **це, швидше за все, не бажано у production**.
У production ви захочете зберегти свої дані, наприклад, у файлі, базі даних або іншому сховищі.

Вам слід скористатися опцією `storage` middleware сесії, щоб підключити його до вашого сховища даних.
...
За замовчуванням сесії зберігатимуться [у вашій пам'яті](#оперативна-памʼять-за-замовчуванням) вбудованим адаптером.
...
За замовчуванням всі дані зберігаються в оперативній памʼяті.
Це означає, що всі сесії будуть втрачені, як тільки ваш бот зупиниться.

Ви можете використовувати клас `MemorySessionStorage` ([довідка API](https://deno.land/x/grammy/mod.ts?s=MemorySessionStorage)) з базового пакета grammY, якщо ви хочете налаштувати додаткові параметри зберігання даних в оперативній памʼяті.
...
> Безкоштовне сховище призначене для використання в проєктах хобі.
> Production застосунки повинні мати власну базу даних.
...
Він не потребує жодних налаштувань - вся автентифікація здійснюється за допомогою вашого токена бота.
...
// Створюємо бота та реєструємо middleware сесії.
const bot = new Bot<MyContext>(""); // <-- помістіть ваш токен бота між ""
...
Відвідайте [це](https://github.com/grammyjs/storages/tree/main/packages#grammy-storages), щоб переглянути список підтримуваних на даний момент адаптерів та отримати інструкції щодо їх використання.

::: tip Ваше сховище не підтримується? Нічого страшного!
Створити власний адаптер сховища надзвичайно просто.
Опція `storage` працює з будь-яким обʼєктом, який підключається до цього [інтерфейсу](https://deno.land/x/grammy/mod.ts?s=StorageAdapter), тому ви можете підключитися до вашого сховища лише кількома рядками коду.
...
Ознайомтеся з відповідними репозиторіями про кожного окремого адаптера.
...
> Якщо ви використовуєте [плагін для конкурентності (runner)](./runner.md), переконайтеся, що ви правильно налаштували `sequentialize`, повернувши з функції **всі** ключі сесії як обмеження.
...
Ось чому ваш flavor для контексту не змінюється.
...
3. Middleware завершує роботу негайно.
...
5. Middleware завершує роботу, виконавши читання та запис до сховища даних.
...
Якщо ви ніколи не зверталися до `ctx.session`, ніяких операцій не буде виконано, але щойно ви звернетеся до властивості `session` об'єкта контексту, буде викликано операцію читання.
...
2. Використовуйте `lazySession` замість `session` для реєстрації вашого middleware сесії.
3. Завжди використовуйте `await ctx.session` замість `ctx.session` скрізь у ваших middleware як для читання, так і для запису.
...
У коді плагіна просто постійно очікуйте на `ctx.session`: якщо буде передано обʼєкт, який не є `Promise`, він буде просто обчислений для себе; отже, ви фактично пишете код лише для лінивих сесій, проте автоматично підтримуєте строгі сесії.
:::
...
Ви можете вибрати будь-які номери JavaScript як версії.
