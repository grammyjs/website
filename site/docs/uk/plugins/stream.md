...
Плагін додає метод [`ctx.replyWithStream`](/ref/stream/streamcontextextension#replywithstream) до [обʼєкта контексту](../guide/context).
...
1. LLM генерують _імовірнісний_ Markdown.
   ...
   Зокрема, **LLM не завжди генерують Markdown, сумісний із Telegram**.
...
3. LLM іноді генерують форматування, яке не підтримується Telegram (навіть якщо ви інструктуєте їх не робити цього).
   Наприклад, більшість LLM _обожнюють_ **таблиці, марковані та нумеровані списки**.
...
- Використовуйте потоковий парсер Markdown і створюйте власні масиви [`MessageEntity`](https://core.telegram.org/bots/api#messageentity) для форматування кожного [`MessageDraftPiece`](/ref/stream/messagedraftpiece).
