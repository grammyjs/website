...
В обʼєкті контексту `getFile` є [скороченим методом](./context.md#скорочені-методи) і отримуватиме інформацію для файлу в поточному повідомленні.
...
Конструктор `InputFile` не лише приймає шляхи до файлів, але й потоки, обʼєкти `Buffer`, асинхронні ітератори та, залежно від вашої платформи, багато іншого.
...
Під час надсилання файлів ви можете вказати додаткові параметри в обʼєкті параметрів типу `Other`, як пояснювалося [раніше](./basics.md#надсилання-повідомлень).
