...
> Якщо ви хочете знати, як ви можете працювати з файлами в grammY, прокрутіть униз до розділу про завантаження файлів [на сервер](#%D0%BE%D1%82%D1%80%D0%B8%D0%BC%D0%B0%D0%BD%D0%BD%D1%8F-%D1%84%D0%B0%D0%B8%D0%BB%D1%96%D0%B2) та [з серверу](#%D0%BD%D0%B0%D0%B4%D1%81%D0%B8%D0%BB%D0%B0%D0%BD%D0%BD%D1%8F-%D1%84%D0%B0%D0%B8%D0%BB%D1%96%D0%B2).
...
Кожного разу, коли ваш бот **отримує** повідомлення з файлом, він фактично не отримуватиме повні дані файлу, а лише `file_id`.
Якщо ваш бот дійсно хоче завантажити файл, він може зробити це, викликавши метод `getFile` ([довідка Telegram Bot API](https://core.telegram.org/bots/api#getfile)).
Цей метод дозволяє завантажити файл, створивши спеціальну тимчасову URL-адресу.
Зауважте, що ця URL-адреса гарантовано дійсна лише протягом 60-ти хвилин, після чого вона може закінчитися. У цьому випадку ви можете просто знову викликати `getFile`.
...
Це означає, що всі файли, які бачить бот, під час надсилання та отримання нададуть `file_id` боту.
...
Щоб дізнатися, як завантажити власні файли, [прокрутіть униз](#надсилання-файлів).
Ви можете повторно використовувати той самий `file_id` скільки завгодно, тож ви можете надіслати той самий файл до п’яти різних чатів, використовуючи той самий `file_id`.
Однак ви повинні переконатися, що використовуєте правильний метод: наприклад, ви не можете використовувати `file_id`, який ідентифікує фотографію, під час виклику [`sendVideo`](https://core.telegram.org/bots/api#sendvideo).
...
**Однак** небезпено використовувати зовнішній `file_id`, оскільки він може перестати працювати в будь-який момент без попередження.
Отже, завжди переконуйтеся, що будь-які `file_id`, які ви використовуєте, доступні вашому боту.
:::

З іншого боку, бот можливо зрештою побачить той самий файл, ідентифікований різними `file_id`.
...
В об’єкті контексту `getFile` є [скороченим методом](./context.md#скорочені-методи) і отримуватиме інформацію для файлу в поточному повідомленні.
Якщо ви хочете отримати інший файл під час обробки повідомлення, використовуйте натомість `ctx.api.getFile(file_id)`.
:::

> Перегляньте [скорочення `:media` та `:file`](./filter-queries.md#скорочення) для запитів фільтрування, якщо ви хочете отримати будь-який файл.

Викликавши `getFile`, ви можете використати повернутий `file_path`, щоб завантажити файл за цією URL-адресою `https://api.telegram.org/file/bot<token>/<file_path>`, де `<token>` має бути замінено токеном вашого бота.

::: tip Плагін файлів
grammY не постачається в комплекті з власним завантажувачем файлів, але ви можете встановити [офіційний плагін файлів](../plugins/files.md).
Він дає змогу завантажувати файли за допомогою `await file.download()` і отримувати заповнену URL-адресу для завантаження за допомогою `file.getUrl()`.
:::
...
Залежно від того, який із трьох способів ви виберете для надсилання файлу, параметри цих функцій відрізнятимуться.
...
grammY має гарну підтримку завантаження власних файлів.
...
Конструктор `InputFile` не лише приймає шляхи до файлів, але й потоки, об’єкти `Buffer`, асинхронні ітератори та, залежно від вашої платформи, багато іншого.
Вам потрібно пам’ятати, що треба **створити екземпляр `InputFile` і передати його будь-якому методу для надсилання файлу**.
Екземпляри `InputFile` можна передати всім методам, які приймають надсилання файлів шляхом завантаження з серверу.

Ось кілька прикладів того, як ви можете створювати `InputFile`.

#### Завантаження файлу з диску

Якщо на вашій машині вже є файл, ви можете дозволити grammY завантажити цей файл.
...
#### Завантаження вихідних двійкових даних

Ви також можете надіслати об’єкт `Buffer` або ітератор, який створює об’єкти `Buffer`.
У Deno ви також можете надіслати об’єкти `Blob`.
...
Фактично це не збереже файл на вашому диску.
Натомість grammY пропускатиме дані лише по каналу та зберігає в пам’яті лише невелику їх частину.
Це дуже ефективно.

> Зауважте, що Telegram підтримує завантаження файлу багатьма способами.
...
Під час надсилання файлів ви можете вказати додаткові параметри в об’єкті параметрів типу `Other`, як пояснювалося [раніше](./basics.md#надсилання-повідомлень).
...
Як завжди, подібно іншим методам API ви можете надсилати файли за допомогою `ctx` (найпростіший), `ctx.api` або `bot.api`.
...
Якщо ви хочете підтримувати завантаження файлів розміром до 2000 МБ (максимальний розмір файлу в Telegram) в обидві сторони, ви повинні розмістити власний сервер Bot API на додаток до хостингу вашого бота.
...
Крім того, ви можете переглянути попередній розділ цього посібника про систему Bot API [тут](./api.md).
