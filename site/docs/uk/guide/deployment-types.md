...
grammY підтримує обидва способи, тоді як за замовчуванням використовується тривале опитування.

У цьому розділі спочатку описано, що насправді таке тривале опитування та вебхуки, й так само окреслено деякі переваги та недоліки використання того чи іншого методу розгортання.
...
Іншими словами, є два принципово різні способи розміщення вашого бота та запуску його на якомусь сервері; вони відрізняються способом, яким повідомлення досягають вашого бота і яким можуть бути оброблені grammY.
...
Ваш бот може підтягнути оновлення через тривале опитування або сервери Telegram можуть передати їх вашому боту через вебхуки.
...
_Уявіть, що ви купуєте собі кульку морозива у своєму надійному кафе-морозиві.
...
Якщо там немає повідомлень, Telegram поверне порожній список, вказуючи, що з моменту останнього запиту вашому боту не було надіслано нових повідомлень.

Коли grammY надсилає запит до Telegram і тим часом вашому боту надсилаються нові повідомлення, Telegram повертає їх у вигляді масиву до 100 об’єктів оновлення.
...
Якщо повідомлень немає, Telegram залишатиме з’єднання відкритим до надходження нових повідомлень, а потім відповідатиме на запит цими новими повідомленнями.

_Знову час для морозива!
Співробітник вже вітає вас.
На запитання про морозиво вашого улюбленого сорту співробітник посміхається вам і завмирає.
Насправді ви взагалі не отримуєте жодної відповіді.
Тому ви вирішуєте почекати, твердо посміхаючись у відповідь.
Ви чекаєте.
Чекаєте._

_За кілька годин до наступного сходу сонця прибуває вантажівка місцевої компанії з доставки їжі і привозить пару великих ящиків у складське приміщення кафе.
...
> Зауважте, що насправді жодне з’єднання не буде відкритим годинами.
> Запити на тривале опитування за замовчуванням мають 30-ти секундний тайм-аут, щоб уникнути ряду [технічних проблем](https://datatracker.ietf.org/doc/html/draft-loreto-http-bidirectional-07#section-5.5).
...
Налаштування **вебхуку** означає, що ви надасте Telegram URL-адресу, доступну з публічного Інтернету.
Щоразу, коли вашому боту надсилається нове повідомлення, Telegram, а не ви, бере на себе ініціативу та надсилає запит із об’єктом оновлення на ваш сервер.
Гарно, а?

_Ви вирішуєте піти в кафе з морозивом востаннє.
Ви кажете своєму другові за прилавком, де ви живете.
Він обіцяє особисто завітати до вашої квартири, коли приїде нове морозиво, щоб воно не розтало на пошті.
Класний хлопець._
...
Вам не потрібно постійно тримати мережеве з’єднання відкритим.
...
- На безсерверних платформах, наприклад, у хмарних функціях або програмованих граничних мережах.

## Я все ще не знаю, що використовувати

Тоді переходьте до тривалого опитування.
...
З grammY вам потрібно торкнутися лише кількох рядків коду.
Налаштування ваших [middleware](./middleware.md) однакові.
...
Якщо ви хочете, щоб ваші повідомлення оброблялися grammY одночасно, або ви турбуєтеся про пропускну здатність, перегляньте розділ про [плагін для конкурентності (runner)](../plugins/runner.md).

## Як використовувати вебхуки

Якщо ви хочете запустити grammY з вебхуками, ви можете інтегрувати свого бота у вебсервер.
Тому ми очікуємо, що ви зможете запустити простий вебсервер з використанням обраного вами фреймворку.

Кожного бота grammY можна перетворити на middleware для багатьох серверних фреймворків, зокрема `express`, `koa`/`oak` тощо.
Ви можете імпортувати функцію `webhookCallback` ([довідка API](https://deno.land/x/grammy/mod.ts?s=webhookCallback)), щоб створити middleware для відповідного фреймворку.
...
// Обов’язково вкажіть структуру, яку ви використовуєте.
...
Обов’язково прочитайте [чудовий путівник Марвіна про все, що стосується вебхуків](https://core.telegram.org/bots/webhooks), написаний командою Telegram, якщо ви плануєте запустити свого бота на вебхуках у VPS.
...
Другий параметр, переданий у `webhookCallback` ([довідка API](https://deno.land/x/grammy/mod.ts?s=webhookCallback)) визначає адаптер фреймворку, який використовується для зв’язку з фреймворком.
...
Як перевагу, це позбавляє вашого бота від створення одного запиту HTTP на оновлення.
Однак у використання цього є ряд недоліків:

1. Ви не зможете обробляти потенційні помилки відповідного виклику API.
   Це включає помилки обмеження швидкості, тому ви не можете бути впевнені, що ваш запит матиме якийсь ефект.
2. Що ще важливіше, ви також не матимете доступу до об’єкта відповіді.
...
4. Зауважте також, що типи в grammY не відображають наслідків виконаного зворотного виклику вебхуку!
   Наприклад, вони вказують на те, що ви завжди отримуєте об’єкт відповіді, тому ви несете відповідальність за те, щоб переконатися, що бот не зламалався, використовуючи цю незначну оптимізацію продуктивності.
...
> Ви можете ігнорувати решту цієї сторінки, якщо всі ваші middleware завершуються швидко, тобто протягом кількох секунд.
...
Telegram має тайм-аут для кожного оновлення, яке він надсилає на вашу кінцеву точку вебхуку.
...
Ось чому grammY має власний коротший тайм-аут всередині `webhookCallback`: за замовчуванням 10 секунд.
Якщо ваш middleware закінчиться раніше, функція `webhookCallback` автоматично відповість на вебхук.
У такому випадку все добре.
Однак, якщо ваш middleware не завершить роботу до закінчення часу очікування grammY, `webhookCallback` видасть помилку.
...
Ви можете налаштувати `webhookCallback` так, щоб не видавати помилку після тайм-ауту, а натомість завчасно завершувати запит вебхуку, навіть якщо ваш middleware все ще працює.
...
Пам’ятайте, що як тільки ви відповісте на запит вебхуку, Telegram надішле наступне оновлення для цього чату.
...
Це може призвести до умов гонки.
Наприклад, плагін сеансу неминуче вийде з ладу через небезпеку [запису після читання](https://en.wikipedia.org/wiki/Hazard_(computer_architecture)#Write_after_read_(WAR)).
**Це спричиняє втрату даних!**
Інші плагіни і навіть ваш власний middleware також можуть зламатися.
Ступінь цього невідома та залежить від вашого бота.

#### Як вирішити цю проблему

Цю відповідь легше сказати, ніж зробити.
**Це ваша робота — переконатися, що ваші middleware завершують роботу досить швидко.**
Не використовуйте довготривалі middleware.
Так, ми знаємо, що ви, можливо, _хочете_ мати довгострокові завдання.
Стійте.
Не робіть цього.
Не у вашому middleware.

Замість цього використовуйте чергу; існує багато систем обробки черг: від дуже простих до дуже складних.
Замість того, щоб намагатися виконати всю роботу в невеликому вікні тайм-ауту вебхуку, просто додайте завдання до черги, яке буде оброблятися окремо, і дайте middleware завершити роботу.
Черга може використовувати весь час, який забажає.
Коли це буде зроблено, він може надіслати повідомлення назад у чат.
Це легко зробити, якщо ви просто використовуєте вбудовану в пам'ять чергу.
...
Умови гонки дуже важко відтворити, і вони можуть виникати надзвичайно рідко або епізодично.
Рішення цього полягає в тому, щоб спочатку _переконатися, що не натрапили на тайм-аути_.
Але якщо ви це зробите, ви дійсно хочете знати, що це відбувається, щоб ви могли дослідити та вирішити проблему!
Тому ви хочете, щоб помилка з’являлася у ваших журналах.
Встановлення обробника тайм-ауту на `"return"`, тобто придушення тайм-ауту та вдавання, ніби нічого не сталося, є прямою протилежністю корисної поведінки.

Якщо ви це зробите, ви в певному сенсі використовуєте чергу оновлень у доставці вебхуку Telegram як чергу завдань.
Це погана ідея з усіх причин, описаних вище.
Те, що grammY _може_ придушувати помилки, через які ви можете втратити свої дані, не означає, що ви _маєте_ зробити це.
Цей параметр конфігурації не слід використовувати у випадках, коли виконання ваших middleware просто займає надто багато часу.
Витратьте час, щоб правильно вирішити цю проблему, тоді майбутні ви та ваші користувачі будуть вам вдячні.
