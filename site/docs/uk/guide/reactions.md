...
У тих же випадках боти також можуть реагувати на повідомлення користувацькими емодзі. хоч боти і не можуть мати [Telegram Premium](https://telegram.org/faq?setln=uk).
...
Дозвольте [плагіну емодзі](../plugins/emoji.md#useful-data-for-reactions) допомогти вам!
:::
...
```ts
bot.reaction(
  { type: "custom_emoji", custom_emoji_id: "рядок-ідентифікатор" },
  async (ctx) => {/* ... */},
);
```
...
grammY надає вам можливість ще більше відфільтрувати оновлення за допомогою спеціальних [запитів фільтрування](./filter-queries.md) для типу реакції.

```ts
// Оновлення, поточна реакція яких містить принаймні один емодзі.
...
Існує [скорочений метод контексту](./context.md#скорочені-методи) під назвою `ctx.reactions`, який дозволяє побачити, як саме змінилася реакція.

Ось як ви можете використовувати `ctx.reactions`, щоб виявити, чи видалив користувач свій голоз "за", але пробачити йому, якщо він все ще зберігає реакцію "ОК".
...
const {
  ...
  emoji,
  /** Емодзі, які нещодавно додані до реакції цього користувача */
  emojiAdded,
  ...
  customEmoji,
  /** Користувацьці емодзі, які нещодавно додані до реакції цього користувача */
  customEmojiAdded,
