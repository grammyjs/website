...
Ага!
Middleware може бути функцією або обʼєктом.
Поки що ми використовували лише функції (`(ctx) => { ... }`), тому давайте наразі залишимо обʼєкти middleware та заглибимося в тип `MiddlewareFn` ([довідка](https://deno.land/x/grammy/mod.ts?s=MiddlewareFn)):
...
Не соромтеся використовувати цей middleware для свого обʼєкта бота, зареєструйте більше обробників і пограйтеся з прикладом.
...
Існує два типи middleware: функції та обʼєкти.
Обʼєкти middleware є просто оболонкою для функцій middleware.
...
composer.use(/*...*/);
composer.use(/*...*/);
bot.use(composer); // composer — обʼєкт middleware!
