...
> Метод `ctx.reply` --- це лише псевдонім для `ctx.api.sendMessage`, дивіться [наступний розділ](./context#доступні-дії).
...
Обробка файлів пояснюється більш детально в [пізнішому розділі](./files#надсилання-файлів).
