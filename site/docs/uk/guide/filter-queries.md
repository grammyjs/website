...
[Пам’ятайте](./context.md#%D0%B4%D0%BE%D1%81%D1%82%D1%83%D0%BF%D0%BD%D1%96-%D0%B4%D1%96%D1%96), що `ctx.msg` надає вам доступ як до повідомлень, так і до дописів каналів, залежно від того, що відповідає запиту.
...
Механізм запитів grammY дозволяє визначити вишукані скорочення, які групують пов’язані запити.
...
Інакше кажучи, використання `edit` еквівалентно прослуховуванню подій `edited_message` та `edited_channel_post`.
...
Інакше кажучи, використання `:media` еквівалентно прослуховуванню подій `:photo` та`:video`.
...
Інакше кажучи, використання `:file` еквівалентно прослуховуванню подій `:photo`, `:animation`, `:audio`, `:document`, `:video`, `:video_note`, `:voice` та `:sticker`.
...
Якщо ви хочете встановити якусь middleware після запитів фільтрування, поєднаних операцією АБО, вам треба передати запити в `bot.on()` як масив.
...
Middleware буде виконано, якщо оновлення відповідає _будь-якому із зазначених запитів_.
Порядок запитів не має значення.

### Поєднання запитів операцією І

Якщо ви хочете встановити якусь middleware після запитів фільтрування, поєднаних операцією І, вам треба створити ланцюжок викликів `bot.on()` з відповідними запитами.
...
Деякі з них є дещо специфічними і знадобляться не всім, тому можете сміливо переходити до [наступного розділу](./commands.md).
...
2. Автоматичні пересилання з пов’язаних каналів у групи для обговорення.
...
Наприклад, ви можете визначити функцію обробки, яка приймає певні об’єкти контексту, які вже були деяким чином відфільтровані:
