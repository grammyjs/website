...
Пояснення: коли ваш бот надсилає повідомлення, воно надсилається як HTTP запит на _сервер Bot API_.
...
Це можна зробити за допомогою [тривалого опитування або вебхуків](./deployment-types.md).

Ви також можете розмістити сервер Bot API на своєму сервері.
Це в основному корисно для надсилання великих файлів або для зменшення затримок.
...
> Зауважте також, що якщо ви маєте доступ до обʼєкта контексту, тобто перебуваєте всередині обробника повідомлень, краще завжди викликати `ctx.api` або одну з [доступних дій](./context.md#доступні-діі).
...
3. Купіть [VPS](../hosting/comparison#vps) у місці розташування вашого дата-центру.
