...
Обʼєкт `Context` ([довідник API grammY](/ref/core/context)) є важливою частиною grammY.
...
> Перегляньте [розділ про реакції](./reactions), якщо вони вас зацікавили.
...
Зауважте, що зазвичай слід використовувати [фільтрування запитів](./filter-queries) і подібні методи.
Використання has перевірок найкраще підходить для використання в [плагіні розмов](../plugins/conversations).
:::
...
Перегляньте [довідку API про обʼєкт контексту](/ref/core/context#has), щоб побачити список усіх has перевірок.
Також перегляньте статичну властивість `Context.has` у [довідці API](/ref/core/context#has), яка дозволяє створювати ефективні предикатні функції для дослідження багатьох обʼєктів контексту.
...
Отже, усі методи обʼєкта контексту приймають обʼєкти параметрів типу `Other`, як пояснювалося [раніше](./basics#надсилання-повідомлень).
...
Якщо ви подивитеся у [довіднику Bot API](https://core.telegram.org/bots/api#sendmessage), що може робити `sendMessage`, ви побачите кілька параметрів: `parse_mode`, `link_preview_options` і `reply_parameters`.
...
Якщо ви хочете отримати загальне уявлення про те, які короткі методи існують, тоді автодоповнення стане вашим другом разом із [довідкою API grammY](/ref/core/context).
...
Той самий обʼєкт контексту для одного оновлення буде спільно використовуватися всіма встановленими на боті проміжними обробниками ([документація](./middleware)).
...
::: tip Проміжний... що?
Цей розділ потребує розуміння проміжних обробників, тому, якщо ви ще не переходили до [розділу про проміжні обробники](./middleware), ось дуже короткий підсумок.
...
Звичайно, власний тип контексту також можна передати іншим речам, які працюють з проміжними обробниками, наприклад в [Composer](/ref/core/composer).

```ts
const composer = new Composer<MyContext>();
```

Деякі плагіни також вимагатимуть передачі власного типу контексту, як-от плагін [маршрутизатор](../plugins/router) або плагін [інтерактивних меню](../plugins/menu).
...
Наприклад, якщо у вас є [дані сесії](../plugins/session), ви повинні зареєструвати `ctx.session` для типу контексту.
Інакше:
...
Тип `SessionFlavor` ([довідка API](/ref/core/sessionflavor)) простий: він визначає лише властивість `session`.
...
Зверніть увагу, що ви повинні передавати `MyContext` не тільки вашому екземпляру `Bot`.
Вам також потрібно використовувати його у багатьох інших місцях.
Наприклад, якщо ви створюєте новий екземпляр `Composer`, використовуєте плагіни на кшталт [маршрутизатора](../plugins/router) або [виносите проміжні обробники в окремі функції](../advanced/structuring#визначення-типів-для-окремого-проміжного-обробника), вам потрібно буде вказати власний тип контексту.
