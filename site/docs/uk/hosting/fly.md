...
6. Виконайте 2-й та 4-й крок з [1-го методу](#1-и-метод-за-допомогою-flyctl) вище.
   Не забудьте пропустити останній, 5-й, крок, оскільки ми не розгортаємо код безпосередньо.
...
замініть `<token>` на токен вашого бота, а `<url>` - на повну URL-адресу вашого застосунку разом з шляхом до обробника вебхуку.

### Оптимізація Dockerfile

При запуску `Dockerfile` він копіює все з каталогу в образ Docker.
Для застосунків Node.js деякі каталоги, як-от `node_modules`, будуть в будь-якому випадку перебудовані, тому їх не потрібно копіювати.
Створіть файл `.dockerignore` і додайте до нього `node_modules`, щоб досягти цього.
