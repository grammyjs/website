...
[Cloudflare Workers](https://www.cloudflare.com/products/workers/) --- це загальнодоступна безсерверна обчислювальна платформа, яка пропонує зручне та просте рішення для запуску JavaScript за допомогою парадигми [кордонних обчислень](https://uk.wikipedia.org/wiki/Кордонні_обчислення).
...
Cloudflare Workers надають нам безпечний спосіб зберігати конфіденційну інформацію, як-от ключі API та токени авторизації, у змінній оточення [secrets](https://developers.cloudflare.com/workers/configuration/secrets/#secrets-on-deployed-workers)!
...
Отже, [згідно з Cloudflare](https://developers.cloudflare.com/workers/configuration/secrets/#local-development-with-secrets), ви можете створити файл `.dev.vars` у корені вашого проєкту для визначення секретних змінних:
