...
4. **Назва сервера.**
   Виберіть будь-яке імʼя, яке вам подобається.
5. **Root-пароль.**
...
```sh
scp <шлях-до-кореня-проєкту> root@<ip-адреса>:<шлях-до-віддаленого-каталогу>
```

Замініть `<шлях-до-кореня-проєкту>` на шлях до локальних файлів, `<ip-адреса>` на IP-адресу вашого сервера, а `<шлях-до-віддаленого-каталогу>` на шлях до каталогу, в якому має зберігатися вихідний код бота на сервері.
