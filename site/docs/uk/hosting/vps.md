...
> Щоб слідувати цьому посібнику, спершу вам потрібно орендувати VPS.
> У цьому першому розділі пояснюється, як це зробити.
> Якщо у вас вже є VPS для роботи, перейдіть до [наступного розділу](#запуск-бота).

Ми обрали [Hostinger](https://hostinger.com.ua) в якості нашого хостера.

> Ви можете вільно обирати провайдера на свій розсуд.
...
Типовий строк оренди --- 1 рік (маркетингова хитрість), і коштує це величезних грошей.
Вам цього напевно не треба, тому для початку орендуйте сервер на місяць, що коштує значно дешевше.

У будь-якому випадку Hostinger надає 30-денну гарантію повернення грошей.
:::

Після оплати ви зможете налаштувати свій сервер:

1. **Місцезнаходження.**
   Ми рекомендуємо вам вибрати розташування найближче до Амстердама.
   Це пов'язано з тим, що там знаходиться головний сервер Bot API.
   Якщо ви використовуєте власний сервер Bot API, виберіть найближче до нього розташування.
2. **Тип сервера.**
...
6. **SSH-ключ.**
   Пропустіть цей крок.
   Ми налаштуємо SSH-ключі [пізніше](#ssh-ключі).
...
У цьому посібнику всі дії з сервером будуть виконуватися від імені користувача root.
Це зроблено навмисно для спрощення викладу матеріалу.
Однак, насправді, користувач root повинен відповідати лише за загальні сервіси (веб-сервер, база даних тощо), а додатки повинні запускатися окремими користувачами, які не є користувачами root.
Такий підхід забезпечує безпеку конфіденційних даних і запобігає злому всієї системи.
...
Наразі ви можете скопіювати локальні файли на віддалений сервер за допомогою такої команди:

```sh
scp <шлях-до-локальних-файлів> root@<ip-адреса>:<шлях-до-віддаленого-каталогу>
```

> Замініть `<шлях-до-локальних-файлів>` на шлях до локальних файлів, `<ip-адреса>` на IP-адресу вашого сервера, а `<<шлях-до-віддаленого-каталогу>` на шлях до каталогу, в якому має зберігатися вихідний код бота на сервері.
...
3. Ваша команда запуску повинна виглядати наступним чином:
   ...
   ```sh
   <повний-шлях-до-середовища-викоання> <опції> <повний-шлях-до-вхідного-файлу>
...
3. Додайте наступний вміст:
   ...
   - `Restart=on-failure` --- вказує на те, що програма повинна перезапускатися після збою.
   - `WantedBy=multi-user.target` --- визначає стан системи, в якому повинен запускатися сервіс.
     `multi-user.target` --- типове значення для серверів, яке визначає стан системи, коли працює все, окрім графічного інтерфейсу, якого просто не існує.

   > Для отримання додаткової інформації про файли служб, прочитайте [це] [цей сайт](https://access.redhat.com/documentation/te-in/red_hat_enterprise_linux/9/html/using_systemd_unit_files_to_customize_and_optimize_your_system/assembly_working-with-systemd-unit-files_working-with-systemd).
...
[PM2](https://pm2.keymetrics.io/) — це демонічний менеджер процесів для Node.js, який допоможе вам керувати та тримати ваш застосунок в режимі онлайн 24/7.

> PM2 створений спеціально для запуску застосунків, які написані на Node.js.
> Проте його також можна використовувати для запуску застосунків, які написані на інших мовах або для інших фреймворків.
...
Якщо ви створили новий застосунок і хочете його також зберегти, просто виконайте `pm2 save` ще раз.
...
Перш ніж ваш домен зможе працювати з вашим VPS, вам потрібно направити домен на ваш сервер.
...
> Якщо у вас виникнуть проблеми, скористайтеся іншим способом, описаним в [базі знань](https://support.hostinger.com/uk/articles/1583227-%D1%8F%D0%BA-%D0%BD%D0%B0%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D1%82%D0%B8-%D0%B4%D0%BE%D0%BC%D0%B5%D0%BD-%D0%BD%D0%B0-vps).
...
Тепер, якщо ви відкриєте у браузері IP-адресу свого сервера, то побачите типову сторінку з інструкцією з налаштуванню Caddy.
...
> Замініть `<домен>` на ваш домен, а `<токен-бота>` на токен бота.

Перезавантажуйте Caddy щоразу, як змінюєте файл конфігураціїї сайту, за допомогою наступної команди:
...
Однак при налаштуванні конвеєру ви можете швидко використати їх усі, через що вам доведеться платити гроші за додаткові ресурси, інакше завдання не виконуватимуться.
Щоб такого не сталося, ми рекомендуємо встановити на вашому компʼютері self-hosted runner, щоб завдання виконувалися на вашому компʼютері.
Так ви позбудетеся обмежень і зможете конвеєри будь-якої складності та потужності (майже).
:::
...
Ця команда згенерує публічний та приватний ключі потрібного для GitHub та GitLab типу та формату.
