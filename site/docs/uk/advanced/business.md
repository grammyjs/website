...
У ньому також вказані дозволи бота в чаті, за допомогою яких можна, наприклад, перевірити, чи має бот право надсилати повідомлення в чат.
...
bot.on("business_message", async (ctx) => {
  ...
  if (ctx.from.id === employee.id) {
    ...
    if (conn.rights?.can_reply) {
      // Ви можете відповісти на це повідомлення.
    }
  }
});
```
...
Ви також можете позначати повідомлення як прочитані та видаляти отримані або надіслані повідомлення в чаті, якщо у бізнес-підключенні надано відповідні дозволи.

```ts
bot.on("business_message").filter(async (ctx) => {
  const conn = await ctx.getBusinessConnection();
  // Видалити всі повідомлення, надіслані клієнтом, що містять фразу "видаліть це".
  if (
    ctx.from.id !== conn.user.id &&
    ctx.msg.text.includes("видаліть це") &&
...
Однак, ваш бот **НЕ** може пересилати повідомлення з чату або копіювати їх куди-небудь.
