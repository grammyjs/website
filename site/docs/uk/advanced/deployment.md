...
2. Використовуйте `await` для всіх `Promise` і встановіть **лінтинг** з правилами, які забезпечують це, щоб ви ніколи не забували.

## Надсилання повідомлень

1. Надсилайте файли, передаючи шлях або `Buffer` замість потоку, або принаймні переконайтеся, що ви [знаєте підводні камені](./transformers.md#випадки-використання-перетворювачів).
...
3. Використовуйте [плагін `transformer-throttler`](../plugins/transformer-throttler.md), щоб уникнути перевищення ліміту швидкості.
4. **Необовʼязково**. Розгляньте можливість використання [плагіна `auto-retry`](../plugins/auto-retry.md) для автоматичного оброблення помилок, викликаних невдалими запитами, та повторного надсилання запиту.
...
2. [Використовуйте `sequentialize` з тією самою функцією вирішувача ключів сеансів, що і middleware сеансу.](./scaling.md#паралелізм-важкий)
...
4. Подумайте про реалізацію [graceful shutdown](./reliability.md#graceful-shutdown), щоб зупинити бота, коли ви захочете його вимкнути, тобто перейти на нову версію.
...
1. Переконайтеся, що ви не виконуєте довготривалі операції у middleware: наприклад, передача великих файлів.
   ...
   Замість цього розгляньте можливість використання системи черг завдань.
2. Ознайомтеся з конфігурацією `webhookCallback` ([довідка API](https://deno.land/x/grammy/mod.ts?s=webhookCallback)).
3. Якщо ви налаштували параметр `getSessionKey` для вашого сеансу, [використовуйте `sequentialize` з тією ж функцією вирішувача ключів сеансів, що і middleware сеансу](./scaling.md#паралелізм-важкий).
...
2. Визначте та надішліть зразки обʼєктів оновлення вашому боту за допомогою `bot.handleUpdate` ([довідка API](https://deno.land/x/grammy/mod.ts?s=Bot#method_handleUpdate_0)).
   Подумайте про те, щоб надихнутися цими [обʼєктами оновлення](https://core.telegram.org/bots/webhooks#testing-your-bot-with-updates), наданими командою Telegram.
