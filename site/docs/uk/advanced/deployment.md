...
3. Використовуйте [плагін `transformer-throttler`](../plugins/transformer-throttler), щоб уникнути перевищення ліміту кількості запитів.
4. **Необовʼязково**. Розгляньте можливість використання [плагіна `auto-retry`](../plugins/auto-retry) для автоматичного оброблення помилок, викликаних перевищенням ліміту кількості запитів, та повторного надсилання таких запитів.
