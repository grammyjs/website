...
If you run your bot via `bot.start()`, or if you are using [grammY runner](../plugins/runner.html), then you should **install an error handler via `bot.catch`**.
...
The `BotError` object bundles up a thrown error with the corresponding [context object](./context.html) that caused the error to be thrown.
...
> Note that if the Bot API server can be contacted, but it returns `ok: false` for a given method call, a [`GrammyError`](./errors.html#the-grammyerror-object) is thrown instead.
