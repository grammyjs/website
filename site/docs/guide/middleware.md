...
Looking back at our earlier example, we now know why `bot.on(":photo")` was never even checked: the middleware in `bot.on(":text", async (ctx) => { ... })` already handled the update, and it did not call `next`.
