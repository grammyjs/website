...
::: tip i18n vs Fluent
Jangan bingung membedakan plugin ini dengan plugin [fluent](./fluent.md).

Plugin ini merupakan versi upgrade dari plugin [fluent](./fluent.md).
...
Mereka kemudian menghitung huruf yang tersisa (nternationalizatio, 18 huruf) lalu meletakkan digit angka tersebut diantara huruf i dan n, sehingga jadilah _i18n_.
...
Ia ditenagai oleh [Fluent](https://projectfluent.org/)—sebuah sistem localization yang dibuat oleh [Mozilla](https://mozilla.org/).
...
Contohnya, kamu bisa menyimpan locale user di [session](./session.md) kamu.
...
Tetapi, jika kamu tidak memiliki terjemahan untuk bahasa tersebut, maka bahasa bawaan lah yang akan digunakan.
Ada kalanya bot tidak dapat mendeteksi bahasa yang digunakan oleh user, jika itu terjadi, maka bot akan menggunakan bahasa bawaannya juga.
...
Di grammY kita bisa menyimpan data user untuk setiap chat dengan menggunakan [session](./session.md).
...
1. Jika session diaktifkan, baca `__language_code` dari session tersebut.
   Jika locale yang dikembalikan valid, maka gunakan locale tersebut.
...
```ts
function myLocaleNegotiator(ctx: Context) {
...
```ts
function myLocaleNegotiator(ctx) {
  return ctx.session.locale ?? ctx.from?.language_code ?? "en";
}
...
Pesan akan di-render kurang lebih seperti ini:
...
### Memuat Locale dari Sebuah Directori
...
i18n.loadLocaleSync("id", {
  // versi sync
  source: `greeting = Halo { $first_name }!
language-set = Bahasa telah diganti menjadi bahasa Indonesia!`,
});
```

## Placeables Khusus dan Functions

Plugin ini juga bisa membantumu untuk menulis sebuah terjemahan.

[Placeables](https://projectfluent.org/fluent/guide/placeables.html) dan [Functions](https://projectfluent.org/fluent/guide/functions.html) adalah dua fitur utama dari sistem localization Fluent.
Plugin ini menambahkan beberapa placeables dan functions ke dalam konteks terjemahan kamu, sehingga beban kerja untuk mengatur konteks-konteks terjemahan bisa dikurangi.

### Placeables

Placeables digunakan untuk menginjeksi data ke dalam sebuah string terjemahan.
Misalnya, kamu ingin menyapa user menggunakan nama user tersebut di pesan sambutan (welcome message).
Namun, mustahil untuk menulis setiap nama user di file terjemahan kamu.
Untuk itu, kamu bisa menggunakan placeables untuk memasukkan nama mereka ke konteks terjemahan, sehingga isi pesan akan berubah secara dinamis sesuai dengan nama pengirim tersebut.

Berikut contoh penggunaan di `id.ftl`:

```fluent:no-line-numbers
welcome = Halo, { $name }!
```

Kita bisa menentukan sebuah nilai untuk placeable `name` di konteks terjemahan seperti ini:

```ts:no-line-numbers
await ctx.reply(ctx.t("welcome", { name: ctx.from.first_name }))
```

Sekarang, nama depan di isi pesan akan berbeda-beda untuk tiap user.

#### `first_name`

Ini adalah pengganti dari `ctx.from.first_name`.
Jadi, kamu tidak perlu menentukannya di konteks.
Kamu bisa menggunakannya secara langsung seperti ini

```fluent:no-line-numbers
greeting = Halo { $first_name }
```

### Functions

Functions mirip seperti placeables.
Bedanya, functions akan dijalankan dan diganti dengan nilai yang telah dikembalikan di output.

#### `CTX`

Function ini dapat digunakan untuk mengakses property context bot secara langsung dari terjemahan kamu.

```fluent:no-line-numbers
id = Your user ID: { CTX("from.id") }
```

`CTX("from.id")` akan diganti dengan `ctx.from.id`.

Kamu juga bisa menentukan nilai bawaan sebagai argument kedua.
Jika suatu nilai tidak dapat diidentifikasi, maka nilai bawaan tersebut yang akan digunakan.

```fluent:no-line-numbers
id = Last name: { CTX("from.last_name", "Kamu tidak memiliki nama belakang.") }
```
...
Fitur ini akan bermanfaat jika bot kamu memiliki [keyboard custom](./keyboard.md#keyboard-custom) berisi teks yang sudah di-localize.
...
- Referensi API: <https://doc.deno.land/https://deno.land/x/grammy_i18n/mod.ts>
