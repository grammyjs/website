<!-- 
  This file is copied from @theme/NavbarItems.vue. 
...
import { useNavLink, useThemeLocaleData } from '~/composables/index.js'; // `~` is an alias path. See `tsconfig.json`
import { resolveRepoType } from '~/utils/index.js'; // `~` is an alias path. See `tsconfig.json`
/** End of inject import */ 
...
<template>
  <nav v-if="navbarLinks.length" class="navbar-items">
    ...
    <div v-for="(item, index) in navbarLinks" :key="item.text" class="navbar-item">
      <NavbarDropdown v-if="item.children" :item="item" :menuIndex="[index]" />
      <AutoLink v-else :item="item" :currentMenu="[item.text]" :menuIndex="[index]" />
    </div>
    <!-- End of inject -->
  </nav>
</template>
