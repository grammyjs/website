...
Когда пользователь заходит на `https://t.me/имя_вашего_бота?start=payload`, его Telegram-клиент покажет кнопку ЗАПУСК, которая (при нажатии) отправит строку из URL-параметра вместе с сообщением, в данном примере текст сообщения будет `"/start payload"`.
...
Когда пользователь нажмет на кнопку под постом, его клиент Telegram откроет чат с вашим ботом и отобразит кнопку ЗАПУСК, как описано выше.
