...
::: tip Знать, когда использовать has checks Это точно такая же логика, которая
...
1. Мы должны иметь доступ к объекту `bot`. Это означает, что мы должны
   передавать объект `bot` по всей нашей кодовой базе, чтобы получить ответ, что
...
`ctx.yourCustomPropertyName = yourCustomValue` внутри такого обработчика, то
...
Однако это сильно затрудняет (если не делает невозможным) установку плагинов, т.
к. они часто требуют установки расширителей контекста.
