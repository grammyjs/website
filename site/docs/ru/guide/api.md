...
ваш бот grammY <———HTTP———> API Бота <———MTProto———> Telegram
```
...
Пример: `sendMessage` в [ссылка на Telegram Bot API](https://core.telegram.org/bots/api#sendmessage) и в [ссылка на grammY API](/ref/core/api#sendmessage).

### Вызов метода

Вы можете вызывать методы API через `bot.api`, или [эквивалентно](./context#доступные-действия) через `ctx.api`:
...
> Заметьте также, что если у вас есть доступ к объекту контекста (т. е. вы находитесь внутри обработчика сообщений), всегда предпочтительнее вызывать `ctx.api` или одно из [доступных действий](./context#доступные-действия).
...
Как уже упоминалось [ранее](./basics#отправка-сообщений), вы можете указать другие опции в третьем аргументе типа `Other`:
...
Например, некоторые специфические свойства в некоторых специфических методах должны быть `JSON.stringify` перед отправкой.
...
::: warning Неправильная автоподсказка на Node.js
Если вы не измените файл `tsconfig.json`, как описано выше, может случиться так, что ваш редактор кода предложит в автоподсказке импортировать типы из `grammy/out/client` или что-то в этом роде.
...
Возможно, вас заинтересует наше [сравнение хостинг-провайдеров](../hosting/comparison#таблица-сравнения).
:::

Однако может найтись еще более подходящее место для запуска бота, хотя это потребует гораздо больше усилий.

[Помните](#general-information) что API сервер бота на самом деле не содержит вашего бота.
...
> Дополнительные опции можно найти [в репозитории API сервера бота](https://github.com/tdlib/telegram-bot-api#installation).
...
Возьмите этот URL и вставьте его в браузер (не забудьте заменить `<token>` на ваш токен бота):
