...
```asciiart:no-line-numbers
https://api.telegram.org/bot<токен_бота>/setWebhook?url=<webhook_url>
```
...
1. Откройте эту ссылку `https://api.telegram.org/bot<токен_бота>/getMe` в вашем любимом браузере. Рекомендуем [Firefox](https://www.mozilla.org/en-US/firefox/), так как он хорошо справляется с отображением `JSON`.
