...
Например, `filter` просто вызывает `use` с некоторым middleware для ветвления, а `on` просто вызывает `filter` снова с некоторой предикатной функцией, которая сопоставляет обновления с заданным [filter queries](../guide/filter-queries).
...
Пересмотрите раздел о [комбинировании filter queries](../guide/filter-queries#комбинирование-filter-queries) с новыми знаниями и почувствуйте свою новую силу.
