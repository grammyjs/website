...
1. Получите до 100 обновлений через `getUpdates` ([Telegram Bot API Reference](https://core.telegram.org/bots/api#getupdates))
...
Параллельная обработка не поставляется с пакетом ядра grammY.
Вместо этого можно использовать **пакет [grammY runner](../plugins/runner)** для запуска вашего бота.
...
В комплект поставки grammY runner входит промежуточное программное обеспечение `sequentialize()`, которое обеспечивает последовательную обработку обновлений, которые сталкиваются между собой.
...
// Добавьте обычныый middleware, теперь с поддержкой безопасных сессий.
