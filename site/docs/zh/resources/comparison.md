...
换句话说，它没有使用任何竞争项目的代码，但某些框架的用户仍然会感到熟悉。

> 请假设这种比较是存在偏见的，尽管我们试图为你提供一个客观的描述，说明使用 grammY 和使用其他库的优缺点。
> 我们会努力使本文中的内容保持最新。
> 如果你发现有一些信息已经过时了，请使用底部的链接编辑这个页面。

## 与其他 JavaScript 框架的比较

::: tip 请先选择你的编程语言
鉴于你正在阅读 JavaScript 生态系统中的一个框架的文档，你可能正在寻找能在 Node.js（或 Deno）上运行的东西。
但是，如果你不是的话，请向下滚动，比较哪些编程语言适合于 bot 开发。
...
我们现在先重点关注它们，但我们（或者你？）可能会在未来增加其他比较。
...
然而，Telegraf 曾经使用 JavaScript 写的（v3 版本）。
类型注释是手动添加的，并且维护得很差，因此它们是不完整、不正确且过时的。
...
类型安全反过来允许更多高级特性，这些特性从根本上改变了我们对 bot 开发的看法，比如 [可定制的 Bot API](/zh/advanced/transformers.md)。
...
由于摆脱了传统的束缚，grammY 可以对底层的中间件做出实质性的贡献，实现令人兴奋的用例，比如 [filter 查询](/zh/guide/filter-queries.md)，[error 边界](zh/guide/errors.md#error-边界)，[可定制的 Bot API](/zh/advanced/transformers.md)，以及更多。
...
从坏的方面来看，这只在短期内有好处。
把所有东西都放在一个巨大的文件里，用一个原始的 `EventEmitter` 来处理复杂的对象流（又称网络请求），这种想法给 Telegram bot 的世界带来了很多痛苦，当然，也阻碍了一些好的想法的实现。
