...
grammY runner 有一个默认的 source，它可以操作任何 `UpdateSupplier`（[API 参考](/ref/runner/UpdateSupplier.md)）。
这样一个 update supplier 可以直接从 bot 实例中创建。
如果你想自己创建一个，请检查 `createUpdateFetcher`（[API 参考](/ref/runner/createUpdateFetcher.md)）。
...
所有的 sink 都是在 `UpdateConsumer` 上操作的（[API 参考](/ref/runner/UpdateConsumer.md)）。
如果你想自己创建一个，请检查 `Bot` 的 `handleUpdate`（[API 参考](/ref/core/Bot.md#handleUpdate)）。

sink 包含了一个当前正在处理的 updates 的队列（[API 参考](/ref/runner/DecayingDeque.md)）。
...
当你使用 `createRunner`（[API 参考](/ref/runner/createRunner.md)） 创建一个 runner 时，你会获得一个可以控制它的处理器。
例如，它允许你启动和停止它，或者获得一个 Promise，它在 runner 停止时解决。
(这个处理器也会在 `run` 中返回。)
检查 [API 参考](/ref/runner/RunnerHandle.md) 中的 `RunnerHandle`。
