...
这个插件是 [fluent](./fluent.md) 的改进版，它可以在 Deno 和 Node.js 上使用。
:::
...
如果你想找到你所在地区的代码，请查看 [这个列表](https://en.wikipedia.org/wiki/IETF_language_tag#List_of_common_primary_language_subtags)。
...
这个插件可以在国际化的过程中帮助你。
它基于 [Fluent](https://projectfluent.org/）——一个由 [Mozilla](https://mozilla.org/) 开发的本地化系统。
...
然而，还有很多东西可以确定用户的地区。
例如，你可以将用户的地区保存在你的 [会话](./session.md) 中。
...
一般来说，在 grammY 中，我们可以使用 [会话](./session.md) 来存储每次聊天的用户数据。
...
当你 [不使用会话](#不适用会话) 时，如果需要为用户设置地区，你可以使用 `useLocale` 方法。
...
[占位符](https://projectfluent.org/fluent/guide/placeables.html) 和 [函数](https://projectfluent.org/fluent/guide/functions.html) 是 Fluent 本地化系统的两个核心特性。
这个插件将一些占位符和函数注入到你的翻译上下文中，并减少你管理翻译上下文的工作。

### 占位符

占位符用于向翻译字符串中注入数据。
例如，在欢迎信息中，你可以使用用户的名字来问候他们。
但你不能在你的翻译中为每个用户的名字设置消息。
相反，你可以在翻译上下文中传递他们的名字，而消息将根据接收者进行改变。
...
然后，对每个用户，消息将根据他们的名字来改变。

#### `first_name`

它是替代 `ctx.from.first_name` 的预设占位符。
所以，你不需要在你的上下文中传递它。
你可以像这样直接使用它。
...
Functions are like placeables.
But unlike placeables, functions get executed and replaced by the value it returns in the final output.
函数和占位符类似。
但它和占位符不同的是，函数会被执行，并返回一个值，这个值会被替换到翻译中。

#### `CTX`

这个函数可以让你直接从你的翻译中访问 bot 上下文的所有属性。

```fluent:no-line-numbers
id = Your user ID: { CTX("from.id") }
```

`CTX("from.id")` 将被替换为 `ctx.from.id` 的值.

你可以选择在第二个参数中传递默认值。
如果这个值是未定义的，则使用默认值进行替换。
...
当你的 bot 有[自定义 Keyboards](./keyboard.md#自定义-keyboards)，并且它包含本地化的文本时，这个特性就很有用了。
...
- API 参考：<https://doc.deno.land/https://deno.land/x/grammy_i18n/mod.ts>
