...
> [了解更多](./filter-queries.html) 关于使用 `bot.on()` 来筛选请求的内容。
...
同时，请看 [下一节](./context.html)，以了解监听器的上下文对象如何使发送消息变得轻而易举！
...
> 函数 `ctx.reply` 只是 `ctx.api.sendMessage` 的一个别名，见 [下一节](./context.html#可用操作)。
...
文件处理在 [后面的章节](./files.html#sending-files) 中有更深入的解释。
