...
> 这个章节会解释文件在 Telegram bot 中是如何工作的。
> 如果你想了解如何在 grammY 中处理文件，请向下滚动到 [下载](#接收文件) 和 [上传](#发送文件) 文件。

文件与消息是分开存储的。
Telegram 服务器上的文件由 `file_id` 标识，它是一个长字符串。
例如，它看起来可能像 `AgADBAADZRAxGyhM3FKSE4qKa-RODckQHxsoABDHe0BDC1GzpGACAAEC`。

### 接收文件的标识符

> bot 仅接收文件标识符。
> 如果想要获取文件内容，则必须明确发起请求。
...
> 文件标识符**仅适用于你的 bot**。
> 如果另一个 bot 使用你的文件标识符，它可能会随机工作、随机崩溃并随机杀死无辜的小猫。
> :cat: → :skull:

每一个 bot 都有它独立的 `file_id` 集合，集合包含了所有它自己能调用的文件。
你不能可靠地使用你朋友的 bot 的 `file_id`，来访问你的 bot 的文件。
每个 bot 会对同一个文件使用不同的标识符。
这意味着你不能简单地猜测一个 `file_id` 并访问一些随机的人的文件，因为 Telegram 会跟踪哪些 `file_id` 对你的 bot 有效。
