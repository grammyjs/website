...
如果你运行 bot 通过 `bot.start()` 或者你使用 [grammY runner](/zh/plugins/runner.html) 运行, 那么然后你应该**通过 `bot.catch` 安装一个错误处理器**。
...
`BotError` 对象将抛出的错误与导致该错误的对应的 [上下文对象](/zh/guide/context.html) 绑定在一起。
...
> 需要注意的是如果 Bot API 服务器能被链接，但是方法回调返回了 `ok: false`，这时就会抛出 [`GrammyError`](/zh/guide/errors.html#GrammyError对象) 作为代替。
...
> 关于第 2 点，你可能想要跳转到中间件的 [高级解释](/zh/advanced/middleware.html) 中去学习如何在 grammY 中连接中间件。
