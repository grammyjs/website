...
你的 bot 能够处理高负载的消息取决于你有没有让它处于 [长轮询状态运行或者通过使用 webhooks](/zh/guide/deployment-types.html)。
...
相应替代的是， [grammY runner](/zh/plugins/runner.html) 这个包可以在你的 bot 中使用。
...
默认的并发量限制在了500。
如果你想对这个库有更深入的了解，[看这里]((/plugins/runner.html))
...
当然，为了让它能够在高负载下运行良好，你应该熟悉 [怎样去使用 webhooks](../guide/deployment-types.html#how-to-use-1)。
...
发生冲突的主要原因是，当你使用 [sessions](../plugins/session.html)，有可能会发生读后写的风险。
想一下这些事件的顺序：
