...
你的 bot 能够处理高负载的消息取决于你有没有让它处于 [长轮询状态运行或者通过使用 webhook](../guide/deployment-types)。
...
如果你让你的 bot 运行在 webhook 模式下，只要接收到更新它就会并发去处理。
当然，为了让它能够在高负载下运行良好，你应该熟悉 [怎样去使用 webhook](../guide/deployment-types#如何使用-webhook)。
这就意味着你不得不去意识到一些并发带来的后果，详见 [下一小节](#并发是困难的)。

另外，[请记得](../guide/deployment-types#及时结束-webhook-请求) Telegram 将按顺序传递来自同一聊天的更新，但并发传递来自不同聊天的更新。
...
8. Bot 完成处理消息 B，并且写入新的 session 到数据库中，并且覆盖掉在处理消息 A 期间执行的操作。
   数据因为读后写而丢失掉了！
...
大多数其他 web 框架的 session 系统只是简单地接受竞态的风险，因为它们在 web 上并不经常发生。
但是，我们不希望这样，因为 Telegram bot 更有可能遇到对同一会话密钥的并行请求冲突。
