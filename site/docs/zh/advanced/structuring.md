...
3. （可选的）集中提前找出更新，并且以正确的方式去发送这些更新。
   你可能还想查看相关的 `bot.route`（[API 参考](https://doc.deno.land/https://deno.land/x/grammy/mod.ts/~/Composer#route)） 或者 [路由器插件](/zh/plugins/router.html)。
...
你也可以使用 [Error 边界](/zh/guide/errors.html#error-边界) 去处理所有程序模块中可能出现的错误。
...
可选地，如果你能够事先知道哪个中间件可以发挥相应的作用，你也可以使用 [router 插件](/zh/plugins/router.html) 或者去绑定不同的模块。
