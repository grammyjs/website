...
> If you think you are already fine with these, scroll right to [Getting Started](#getting-started).

First, internationalization is a very long word, hence people like to write the first letter (i) and the last letter (n).
...
If you don't hard-code how dates and times are represented, and instead use a library that adjusts these values according to different standards, you are doing internationalization.
...
If you're unfamiliar with the Fluent's syntax, you can read their guide: <https://projectfluent.org/fluent/guide>
...
// Remember to do this before registering the i18n middleware.
bot.use(session({
  initial: () => {
    return {};
  },
...
i18n.loadLocalesDirSync("locales 2");
```
...
await i18n.loadLocale("en", { filePath: "locales/en.ftl" });
i18n.loadLocaleSync("de", { filePath: "locales/de.ftl" });
```
