...
grammY's i18n plugin can help you to get that done as easily as possible.

The i18n plugin is built on top of [Fluent](https://projectfluent.org/).
Fluent is a localization system made by Mozilla Foundation for natural-sounding translations.
...
But, the quickest of them all is having all the translations in a folder and loading that folder.
Usually, the name of that folder is going to be **locales/**.
And the translation file's name should have the extension `.ftl` (fluent).
...
and here is how you can show them to the user:
...
When is received, the best possible translation is selected for the user.
...
Maybe it is better to show the user from the `language_code` we receive from Telegram updates.
...
But this will not directly allow you to control the language selection (Even though, you can just implement a [custom locale negotiator](#custom-locale-negotiation) to achieve that).
...
When sessions are enabled, `__language_code` property in the session has more priority than `ctx.from.language_code`, in language selection.
...
Well, we saw configuring the i18n instance with or without sessions.
Here is how they choose the most appropriate language for the user:
...
Well, this order is great, there could be another situation where you need to take control over the language selection.
...
bot.command("cart", async (ctx) => {
  ...
  await ctx.reply(ctx.t("cart-msg", {
    items: 10,
  }));
});
```
...
### Load locales using `directory` option
...
You can either specify file path to the translation like this,
