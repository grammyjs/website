...
// uncomment to asynchronously load translations in Deno Deploy
...
// Be awared, in Deno Deploy service,
// we can only load fluent scripts after init I18n.
const i18n = new I18n<MyContext>({
  defaultLocale: "en",
  useSession: true, // whether to store user language in session
...
// Register the i18n middleware after session middleware
bot.use(i18n);
