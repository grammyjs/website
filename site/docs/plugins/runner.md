...
grammY runner ships with one default source that can operate on any `UpdateSupplier` ([API reference](/ref/runner/UpdateSupplier.md)).
Such an update supplier is straightforward to create from a bot instance.
If you want make one yourself, be sure to check out `createUpdateFetcher` ([API reference](/ref/runner/createUpdateFetcher.md)).
...
All of them operate on `UpdateConsumer` objects ([API reference](/ref/runner/UpdateConsumer.md)) which are straightforward to create from a bot instance.
...
The sink contains a queue ([API reference](/ref/runner/DecayingDeque.md)) of individual updates that are currently being processed.
...
When you create a runner with `createRunner` ([API reference](/ref/runner/createRunner.md)), you obtain a handle that you can use to control the runner.
...
Check out the [API reference](/ref/runner/RunnerHandle.md) of the `RunnerHandle`.
