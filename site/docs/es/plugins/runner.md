...
El runner de grammY viene con una fuente por defecto que puede operar con cualquier `UpdateSupplier` ([referencia API](/ref/runner/UpdateSupplier.md)).
Dicho proveedor de actualizaciones es sencillo de crear a partir de una instancia del bot.
Si quieres crear uno tú mismo, asegúrate de consultar `createUpdateFetcher` ([referencia de la API](/ref/runner/createUpdateFetcher.md)).
...
Todos ellos operan con objetos `UpdateConsumer` ([referencia de la API](/ref/runner/UpdateConsumer.md)) que son fáciles de crear a partir de una instancia del bot.
Si quieres hacer uno tú mismo, asegúrate de revisar `handleUpdate` en la instancia `Bot` de grammY ([referencia API](/ref/core/Bot.md#handleUpdate)).

El sumidero contiene una cola ([referencia de la API](/ref/runner/DecayingDeque.md)) de actualizaciones individuales que se están procesando actualmente.
...
Cuando creas un runner con `createRunner` ([referencia de la API](/ref/runner/createRunner.md)), obtienes un manejador que puedes usar para controlar el runner.
...
Consulta la [referencia API](/ref/runner/RunnerHandle.md) del `RunnerHandle`.
