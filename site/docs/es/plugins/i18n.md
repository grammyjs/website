...
## Explicación de la internacionalización
...
Este método es lo mismo que especificar `directorio` en las opciones.
...
i18n.loadLocaleSync("de", {
  // sync version
  source: `greeting = Hallo { $first_name }!
language-set = Die Sprache wurde zu Deutsch geändert!`,
});
```

## Placeables y funciones personalizadas

Este plugin también puede ayudarte a escribir traducciones.

Los [Placeables](https://projectfluent.org/fluent/guide/placeables.html) y las [Funciones](https://projectfluent.org/fluent/guide/functions.html) son dos características fundamentales del sistema de localización Fluent.
Este plugin inyecta algunos placeables y funciones en su contexto de traducción y reduce su trabajo de gestión del contexto de traducción.

### Placeables

Los placeables se utilizan para inyectar datos en una cadena de traducción.
Por ejemplo, en un mensaje de bienvenida, puedes usar el nombre del usuario para saludarlo.
Pero no puedes tener mensajes para el nombre de cada usuario en tus traducciones.
En su lugar, puedes pasar su nombre en el contexto de traducción, y el mensaje cambiará según el receptor.

Esto es lo que harías en `en.ftl`:

```fluent:no-line-numbers
welcome = Hi there, { $name }!
```

Podemos pasar un valor para el colocador `name`, en el contexto de la traducción así:

```ts:no-line-numbers
await ctx.reply(ctx.t("welcome", { name: ctx.from.first_name }))
```

Entonces el mensaje será diferente para cada usuario, con sus nombres de pila.

#### `first_name`

Es un sustituto de `ctx.from.first_name`.
Así que no necesitas pasarlo en el contexto que necesites.
Puedes usarlo directamente así
...
Las funciones son como los placeables.
Pero a diferencia de los placeables, las funciones se ejecutan y son reemplazadas por el valor que devuelven en la salida final.

#### `CTX`

Esta función puede ayudarte a acceder a cualquier propiedad del contexto del bot directamente desde tus traducciones.

```fluent:no-line-numbers
id = Your user ID: { CTX("from.id") }
```

El `CTX("from.id")` será reemplazado por `ctx.from.id`.

Opcionalmente, puedes pasar un valor por defecto como segundo argumento.
Si el valor no está definido, se utilizará el valor por defecto.

```fluent:no-line-numbers
id = Last name: { CTX("from.last_name", "No tienes apellido.") }
```

## Escuchar el texto localizado
...
Esta función es muy útil cuando tu bot tiene [teclados personalizados](./keyboard.md#custom-keyboards) que contienen texto localizado.
...
- Referencia de la API: <https://doc.deno.land/https://deno.land/x/grammy_i18n/mod.ts>
