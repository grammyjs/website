...
Si ejecutas tu bot a través de `bot.start()`, o si estás usando [grammY runner](../plugins/runner.html), entonces deberías **instalar un manejador de errores a través de `bot.catch`**.
...
El objeto `BotError` agrupa un error lanzado con el correspondiente [objeto contexto](./context.html) que causó el error.
...
> Ten en cuenta que si se puede contactar con el servidor de la API del bot, pero devuelve `ok: false` para una llamada a un método determinado, se lanza un [`GrammyError`](../guide/errors.html#the-grammyerror-object) en su lugar.
...
> En cuanto al punto 2, es posible que desee saltar a [la explicación avanzada](../advanced/middleware.html) de middleware para aprender cómo funciona el encadenamiento en grammY.
