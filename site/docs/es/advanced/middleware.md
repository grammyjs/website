...
En está guía, [introdujimos el middleware](../guide/middleware.html) como un stack de funciones.
...
Por ejemplo, `filter` sólo llama a `use` con algún middleware de bifurcación, mientras que `on` sólo llama a `filter` de nuevo con alguna función de predicado que compara las actualizaciones con la [consulta de filtro dada](../guide/filter-queries.html).
...
Revisa la sección sobre [combinar consultas de filtro](../guide/filter-queries.html#combining-multiple-queries) con tus nuevos conocimientos y siente tu nuevo poder.
