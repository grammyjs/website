...
3. Comprueba la firma del método para las [acciones disponibles](../guide/context.html#acciones-disponibles) en `ctx` [aquí](https://deno.land/x/grammy/mod.ts?s=Context#Methods), o para los métodos API (`ctx.api`, `bot.api`) [aquí](https://deno.land/x/grammy@/mod.ts?s=Api#Methods).
