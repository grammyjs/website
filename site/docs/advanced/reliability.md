...
If you made sure you have proper [error handling](../guide/errors.html) for your bot, you are basically good to go.
...
> Do you just care about coding a Telegram bot? [Skip the rest of this page.](./flood.html)
...
If you are using the [grammY runner](../plugins/runner.html) in concurrent mode, the next `getUpdates` call is potentially performed before your middleware processes the first update of the current batch.
...
On the other hand, if your bot is under heavy load and the update polling is slowed down due to [the automatic load constraints](../plugins/runner.html#sink), chances are increasing that some updates will be fetched again, which leads to duplicate messages again.
