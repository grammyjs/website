...
3. Perhaps double-check the method signature for [actions](../guide/context.html#available-actions) on `ctx` [here](https://deno.land/x/grammy/mod.ts?s=Context#Methods), or for API methods (`ctx.api`, `bot.api`) [here](https://deno.land/x/grammy@/mod.ts?s=Api#Methods).
